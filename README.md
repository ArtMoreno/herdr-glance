...
Blocked cards wrap text from `--source detection` and show the newest rows beneath `NEEDS YOU`, keeping old shell history from displacing the latest prompt. Long questions can still exceed the card; Enter takes you to the full agent. This is a terminal excerpt, not an extracted question. Native Herdr status is authoritative: Glance does not infer status or extract questions with regexes. Some integrations may report idle during a long-running tool call.
